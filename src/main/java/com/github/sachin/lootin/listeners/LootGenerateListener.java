...
//    used when loot is not generated by player, eg. using hopper to get contents of loot chest
    ...
    public void onLootGenerate(LootGenerateEvent e){
        ...
        if(plugin.getBlackListStructures().contains(e.getLootTable().getKey())) return;
        String loottable = e.getLootTable().getKey().getKey();
        if(e.getInventoryHolder() instanceof DoubleChest){
            ...
            barrel.update(true);
            ChestUtils.setLootinContainer(null,barrel, ContainerType.CHEST);
            new BukkitRunnable(){
                @Override
                public void run() {
                    ChestUtils.setContainerItems(null,barrel,ContainerType.CHEST, Arrays.asList(barrel.getInventory().getContents()), LConstants.DATA_KEY_STRING);
